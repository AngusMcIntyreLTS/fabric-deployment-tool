...
class fdtUtils:
    ...
    def get_mapping_table_new_from_type(self, type):
        result = ""
        filtered_data = list(
            filter(lambda item: item["Type"] == type, self.mapping_table)
        )
    ...
    def get_mapping_table_new_from_old(self, old):
        result = ""
        filtered_data = list(
            filter(lambda item: item["old"] == old, self.mapping_table)
        )
        if len(filtered_data) > 0:
            result = filtered_data[0]["new"]
        return result
    ...
    def get_mapping_table_new_from_type_item(self, type, item):
        ...
        filtered_data = list(
            filter(
                lambda table: table["Type"] == type and table["Item"] == item,
                self.mapping_table,
            )
        )
        if len(filtered_data) > 0:
        ...
        for root, _, files in os.walk(folder_path):
            for file_name in files:
                if file_name.endswith(
                    (".py", ".json", ".pbir", ".platform", ".ipynb", ".py", ".tmdl")
                ) and not file_name.endswith("report.json"):
    ...
    def fab_update_environments_spark_monitor(
        self, evironments, workspace_name, eventstream_name, connection_name
    ):
        ...
        for environment in evironments:
            ...
            self.trg_workspace_id = self.fab_get_workspace_id(
                f"{workspace_name}.Workspace"
            )
            self.trg_workspace_name = f"{workspace_name}.Workspace"
            eventstream_name = (
                f"{eventstream_name}.Eventstream"
                if ".Eventstream" not in eventstream_name
                else eventstream_name
            )
            connection_string = self.fab_get_eventstream_connection_string(
                eventstream_name, connection_name
            )
            ...
            response = self.run_fab_command(
                f"api -X patch /workspaces/{workspace}/environments/{environment}/staging/sparkcompute -i  {StringSparkProperties}",
                capture_output=True,
                silently_continue=True,
            )
            response = self.run_fab_command(
                f"api -X post /workspaces/{workspace}/environments/{environment}/staging/publish ",
                capture_output=True,
                silently_continue=True,
            )
    ...
    def update_capcity_events_eventstream(self, workspace, item_name="CapacityEvents"):
        ...
        with open(property_file, "r", encoding="utf-8") as file:
            ...
            for index, row in dfCapacities.iterrows():
                ...
                filtered_data = list(
                    filter(
                        lambda table: table.get("properties", {}).get("capacityId")
                        == capacity_id,
                        sources,
                    )
                )
